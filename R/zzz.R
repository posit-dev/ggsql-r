...
release_bullets <- function() {
  c(
    ...
    "Smoke-test the externalized vendor path: in a clean checkout, `unlink(tools::R_user_dir('ggsql', 'cache'), recursive = TRUE)`, then `R CMD build .` and `R CMD INSTALL` the resulting tarball — install should download the archive from the release"
  )
}
