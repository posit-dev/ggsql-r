...
After registration, the data frame can be queried by name in SQL
statements. You can use `ggsql_table` to extract tables from the reader
(both registered ones and those native to the backend) and
`ggsql_table_names` to get a vector of all the tables in reader.
...
- replace:
  ...
  If `TRUE`, replace an existing table with the same name. Defaults to
  `FALSE`.

## Value

`reader` for `ggsql_register()` and `ggsql_unregister()`. `ggsql_table`
returns a data.frame if the table exists and `NULL` if not.
